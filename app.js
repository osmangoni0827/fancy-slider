...
const showImages = (images) => {
  ...
  if (images == '') {
    ...
    AllbuttonsPart.classList.add('d-none');
  }
  else {
    AllbuttonsPart.classList.remove('d-none');
...
const selectItem = (event, img) => {
  ...
  const AllClass = element.classList[2];
  if (AllClass === 'added') {
...
const createSlider = () => {
  ...
  if (duration === '') {
    duration = prompt("Please Set Duration");
  }
  if (duration < 0) {
    duration = duration * (-1);
  }
...
//Back To Home From Slider Button Function
